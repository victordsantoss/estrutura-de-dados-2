...
// ======================================================= //
int table_size  = 0;
char table_of_emails[1001][101];

void cleaning(char * clean_email, char * string) {
	bool ignore = false, keep_dots = false;
	
	for(int i=0,j=0; string[i] != '\0' ; i++)
	{
		if(string[i] == '+') { ignore = true; continue; };
		if(string[i] == '@') { ignore = false; keep_dots=true; }
		if(ignore == true) { continue; }
		if(string[i] == '.' && keep_dots == false) { continue; }
		clean_email[j++] =  string[i];

		if(string[i+1] == '\0') { clean_email[j] = '\0'; }
	}
...
int main()
{
	...
	while(N--)
	{
		...
		char clean_email[101];

		cleaning(clean_email, string);

		if (set_contains(&set, clean_email) != SET_TRUE) {
			set_add(&set, clean_email);
			++ANS;
		}
	}
